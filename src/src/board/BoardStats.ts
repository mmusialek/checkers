...
export class BoardStats implements IGameLoopObject {
    ...
    private readonly statStyle = { fontFamily: FontsConst.secondaryFontFamily, color: FontsConst.primaryColor, fontSize: 20 };
    ...
    updateNames() {
        this.blackPawnText?.setText(this.players === 2 ? "Player 2" : "CPU");
    }
    ...
    private initStats() {
        ...
        const turnText = "turn";
        this.turnImg = getNewText({ x: startXTmp, y: startY }, turnText || "x", this.statStyle);
        ...
        this.blackPawnText = getNewText({ x: startPawnXLabel, y: startY }, this.players === 2 ? "Player 2" : "CPU", this.statStyle);

        startY += (this.turnImg.height / 2) + 50;
        this.whitePawn = getNewSprite({ x: startXTmp, y: startY }, GamePawnType.whitePawn);
        this.whitePawnText = getNewText({ x: startPawnXLabel, y: startY }, "Player 1", this.statStyle);
        ...
        this.scoreTextImg = getNewText({ x: startPawnXLabel, y: startYTmp }, scoreText, this.statStyle);

        startYTmp += (this.scoreTextImg.height / 2) + 50;
        this.scoreBlackTextImg = getNewText({ x: startPawnXLabel, y: startYTmp }, "0", this.statStyle);
        startYTmp += (this.scoreTextImg.height / 2) + 50;
        this.scoreWhiteTextImg = getNewText({ x: startPawnXLabel, y: startYTmp }, "0", this.statStyle);
